...
import frappe
import json
from frappe.permissions import update_permission_property  # تأكد من استيراد الدالة
...
class User(Document):
	...
	def after_insert(self):
		try:
			# حاول إضافة الدور
			#self.add_role(self.permissionsuser)
			# إذا نجحت إضافة الدور، قم بتنفيذ تعليمات الإشعارات والتخزين المؤقت
			create_notification_settings(self.name)
			frappe.cache.delete_key("users_for_mentions")
			frappe.cache.delete_key("enabled_users")
			# إذا نجحت تعليمات الإشعارات والتخزين المؤقت، أضف بيانات إلى جدول tabHas Role
			#self.add_has_role(self.permissionsuser)
	...
	def add_role(self, name):
		try:
			...
			frappe.msgprint(f"Role '{name}' added successfully!")
		except Exception as e:
			frappe.throw(f"An error occurred while adding the role: {e}") 

	def add_has_role(self, role_name):
		try:
			...
			frappe.msgprint(f"Has Role '{role_name}' added successfully!")
		except Exception as e:
			frappe.throw(f"An error occurred while adding the has role: {e}")


	def delete_from_add_user_permissions(self):
		if frappe.db.exists("Role", {"name": self.permissionsuser}):
			frappe.delete_doc("Role", self.permissionsuser, ignore_permissions=True)
		if frappe.db.exists("Has Role", {"role": self.permissionsuser}):
			frappe.db.delete("Has Role", {"role": self.permissionsuser})
	...
	def on_trash(self):
		...
		try:
			self.delete_from_add_user_permissions()
		except Exception as e:
			frappe.throw(_("An error occurred while deleting: {0}").format(e))
...
@frappe.whitelist()
def get_docperm_roles():
    # تنفيذ استعلام SQL لإرجاع الأدوار مع منع التكرار
    roles = frappe.db.sql("""
        SELECT DISTINCT role AS name
        FROM `tabDocPerm`
    """, as_dict=True)
    return roles
@frappe.whitelist()
def get_role_permissions_user(role_full_name):
    """
    Retrieves permissions for a specific role from the `tabCustom DocPerm` table.
    """
    if not role_full_name:
        frappe.throw("Role name cannot be empty")

    query = """
        SELECT `parent`, `read`, `write`, `create`, `delete`, `submit`, `cancel`, `amend`, `report`, `export`, `print`
        FROM `tabCustom DocPerm`
        WHERE `role` = %s
    """
    result = frappe.db.sql(query, (role_full_name,), as_dict=True)
    return result

@frappe.whitelist()
def get_parents_by_role(role):
    """جلب بيانات الحقول parent من جدول tabDocPerm بناءً على role"""
    return frappe.get_all("DocPerm", filters={"role": role}, fields=["parent"])

@frappe.whitelist()
def save_permissions(role_name, permissions):
    """حفظ بيانات الصلاحيات في جدول tabCustom DocPerm"""

    # التحقق من وجود الصلاحية باسم role_name في جدول tabRole
    role_exists = frappe.db.get_value("Role", {"name": role_name})
    if not role_exists:
        frappe.throw(frappe._("لا يوجد صلاحية باسم {0}. يرجى إضافة صلاحية باسم {0}".format(role_name)))

    # تحويل سلسلة النصوص إلى كائن JSON إذا كانت سلسلة نصية
    if isinstance(permissions, str):
        permissions = json.loads(permissions)

    # التحقق من طول القيم للتأكد من أنها لا تتجاوز 140 حرفًا
    if len(role_name) > 140:
        frappe.throw(frappe._("The value for 'role_name' exceeds the maximum allowed length of 140 characters"))

    # جمع جميع القيم `parent` الموجودة في `permissions`
    permissions_parents = {state[1] for state in permissions.values()}

    # جلب جميع السجلات الموجودة في جدول `Custom DocPerm` لنفس `role_name`
    existing_parents = frappe.db.get_all(
        "Custom DocPerm",
        filters={"role": role_name},
        fields=["parent"]
    )

    # تحويل القيم إلى قائمة بسيطة
    existing_parents_set = {record["parent"] for record in existing_parents}

    # تحديد القيم التي يجب حذفها
    parents_to_delete = existing_parents_set - permissions_parents

    # حذف السجلات التي يجب حذفها
    if parents_to_delete:
        frappe.db.sql("""
            DELETE FROM `tabCustom DocPerm`
            WHERE `role` = %(role)s AND `parent` IN %(parents)s
        """, {
            "role": role_name,
            "parents": tuple(parents_to_delete)
        })

    # تحديث أو إضافة السجلات الجديدة
    for document_name, state in permissions.items():
        parent = state[1]
        if len(parent) > 140:
            frappe.throw(frappe._("The value for 'parent' exceeds the maximum allowed length of 140 characters"))

        # التحقق من وجود سجل بنفس role و parent
        existing_record = frappe.db.exists("Custom DocPerm", {"role": role_name, "parent": parent})
        if existing_record:
            # تحديث السجل إذا كان موجودًا
            frappe.db.sql("""
                UPDATE `tabCustom DocPerm`
                SET `read` = %(read)s, `write` = %(write)s, `create` = %(create)s, `submit` = %(submit)s,
                    `cancel` = %(cancel)s, `delete` = %(delete)s, `amend` = %(amend)s, `report` = %(report)s,
                    `export` = %(export)s, `print` = %(print)s,
                    `modified` = NOW(), `modified_by` = 'Administrator'
                WHERE `role` = %(role)s AND `parent` = %(parent)s
            """, {
                'role': role_name,
                'parent': parent,
                'read': state[3],
            ...
            INSERT INTO `tabCustom DocPerm` (
                ...
                `idx`, `permlevel`, `role`, `read`, `write`, `create`, `submit`, `cancel`,
                `delete`, `amend`, `report`, `export`, `print`, `if_owner`
            ) VALUES (
                %(name)s, NOW(), NOW(), 'Administrator', 'Administrator', 0, %(parent)s, 1, 0, %(role)s,
                %(read)s, %(write)s, %(create)s, %(submit)s, %(cancel)s, %(delete)s, %(amend)s, %(report)s,
                %(export)s, %(print)s, 0
            )
            """

            # توليد قيمة فريدة للحقل `name`
            name = frappe.generate_hash(length=10)

            # تنفيذ جملة SQL مع القيم المحددة
            frappe.db.sql(sql, {
                'name': name,
                'parent': state[1],
                'role': role_name,
...
@frappe.whitelist()
def update_user_permission(role, doc, perm, checked, user, if_owner):
    if not frappe.db.exists("User", user):
        return {"status": "error", "message": f"User {user} not found"}

    try:
        # البحث عن الصلاحيات المخصصة للمستخدم
        exists = frappe.db.exists("DocPerm", {"role": role, "parent": doc, "custom_user": user})
        
        if exists:
            # تحديث الصلاحية المحددة
            frappe.db.sql("""
                UPDATE `tabDocPerm`
                SET `{perm}` = %s,
                    `modified` = %s,
                    `modified_by` = %s
                WHERE `role` = %s AND `parent` = %s AND `custom_user` = %s
            """.format(perm=perm), (checked, now_datetime(), frappe.session.user, role, doc, user))
        else:
            # توليد قيمة فريدة لحقل name
            unique_name = frappe.generate_hash(length=10)
            # إدخال الصلاحية الجديدة
            frappe.db.sql("""
                INSERT INTO `tabDocPerm` (`name`, `parent`, `parentfield`, `parenttype`, `idx`, `permlevel`, `role`, `if_owner`, `{perm}`, `custom_user`, `creation`, `modified`, `owner`, `modified_by`, `docstatus`)
                VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
            """.format(perm=perm), (
                unique_name, doc, 'permissions', 'DocType', 0, 0, role, if_owner, checked, user, 
                now_datetime(), now_datetime(), frappe.session.user, frappe.session.user, 0
            ))
        
        return {"status": "success", "message": "Permission updated successfully"}
    except Exception as e:
        frappe.log_error(frappe.get_traceback(), "Update User Permission Error")
...
def get_services_data():
    """
    جلب بيانات جميع الخدمات من جميع الدوك تايبات
    وإرجاع عدد كل خدمة مثل SQL الأصلي
    """
    try:
        query = """
            SELECT visa_type, name_item, COUNT(*) AS total_count
            FROM (
                SELECT visa_type, name_item FROM `tabVisa Application`
                WHERE docstatus = 1 AND transaction_type = 'فرد' 
                  AND visa_type IS NOT NULL AND visa_type != ''
                  AND name_item IS NOT NULL AND name_item != ''
                UNION ALL
                SELECT visa_type, name_item FROM `tabResidency Service`
                WHERE docstatus = 1 AND transaction_type = 'فرد'
                  AND visa_type IS NOT NULL AND visa_type != ''
                  AND name_item IS NOT NULL AND name_item != ''
                UNION ALL
                SELECT visa_type, name_item FROM `tabTransport Service`
                WHERE docstatus = 1 AND transaction_type = 'فرد'
                  AND visa_type IS NOT NULL AND visa_type != ''
                  AND name_item IS NOT NULL AND name_item != ''
                UNION ALL
                SELECT visa_type, name_item FROM `tabPassport Service`
                WHERE docstatus = 1 AND transaction_type = 'فرد'
                  AND visa_type IS NOT NULL AND visa_type != ''
                  AND name_item IS NOT NULL AND name_item != ''
                UNION ALL
                SELECT visa_type, name_item FROM `tabVisa Extension`
                WHERE docstatus = 1 AND transaction_type = 'فرد'
                  AND visa_type IS NOT NULL AND visa_type != ''
                  AND name_item IS NOT NULL AND name_item != ''
                UNION ALL
                SELECT visa_type, name_item FROM `tabTravel Insurance`
                WHERE docstatus = 1 AND transaction_type = 'فرد'
                  AND visa_type IS NOT NULL AND visa_type != ''
                  AND name_item IS NOT NULL AND name_item != ''
                UNION ALL
                SELECT visa_type, name_item FROM `tabDocument Attestation`
                WHERE docstatus = 1 AND transaction_type = 'فرد'
                  AND visa_type IS NOT NULL AND visa_type != ''
                  AND name_item IS NOT NULL AND name_item != ''
				UNION ALL
                SELECT visa_type, name_item FROM `tabParcels`
                WHERE docstatus = 1 AND transaction_type = 'فرد'
                  AND visa_type IS NOT NULL AND visa_type != ''
                  AND name_item IS NOT NULL AND name_item != ''
                UNION ALL
                SELECT visa_type, name_item FROM `tabTravel Work Permit`
                WHERE docstatus = 1 AND transaction_type = 'فرد'
                  AND visa_type IS NOT NULL AND visa_type != ''
                  AND name_item IS NOT NULL AND name_item != ''
                UNION ALL
                SELECT visa_type, name_item FROM `tabBeneficiary Data`
                WHERE docstatus = 1
                  AND visa_type IS NOT NULL AND visa_type != ''
                  AND name_item IS NOT NULL AND name_item != ''
                UNION ALL
                SELECT visa_type, name_item FROM `tabAgent Sales Invoice Tabel`
                WHERE docstatus = 1
                  AND visa_type IS NOT NULL AND visa_type != ''
                  AND name_item IS NOT NULL AND name_item != ''
            ) AS all_services
            GROUP BY visa_type, name_item
