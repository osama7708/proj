...
import { workspaceTabs } from "../ui/workspace_tabs";
	...
	get_meta_html(doc) {
		...
		if (this.list_view_settings && !this.list_view_settings.disable_comment_count) {
			comment_count = `<span class="comment-count d-flex align-items-center">
				<link href="https://cdn.jsdelivr.net/npm/bootstrap-icons/font/bootstrap-icons.css" rel="stylesheet">
				<i class="bi bi-eye" style="color: #0097a6; font-size: 16px; font-weight: 700; padding-left: 10px;">عرض</i>
	...
	setup_list_click() {
		this.$result.on("click", ".list-row, .image-view-header, .file-header", (e) => {
			...
			if ($target.is("a")){
				...
				workspaceTabs.openOrActivateSubTab({
					mainTitle,
					mainLabel,
					href: $target[0].href,
...
class ElementFactory {
	...
	create_like_element(doctype) {
		...
		like.innerHTML = `<link href="https://cdn.jsdelivr.net/npm/bootstrap-icons/font/bootstrap-icons.css" rel="stylesheet">
							<i class="bi bi-hand-thumbs-up" style="color: #0097a6;"></i>`;
	...
	get_like_element(name, liked, liked_by, title) {
		...
		if (liked) {
			icon.classList.remove("bi-hand-thumbs-up");  // إزالة الأيقونة السابقة
			icon.classList.add("bi-hand-thumbs-up-fill"); // إضافة الأيقونة الجديدة
		} else {
			icon.classList.remove("bi-hand-thumbs-up-fill"); // إزالة الأيقونة السابقة
			icon.classList.add("bi-hand-thumbs-up");         // إضافة الأيقونة الجديدة
		}
